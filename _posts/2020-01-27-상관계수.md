...
Let $X = (x_1, x_2, \ldots, x_n)$ and $Y = (y_1, y_2, \ldots, y_n)$ be given. Assume that $X$ is increasing. We will show that the inner product $\langle X, Y \rangle = \sum_{i=1}^n x_i \cdot y_i$ of $X$ and $Y$ has the maximum value when $Y$ is also increasing. In other words, $Y_0 = \argmax_Y \langle X, Y \rangle$ if and only if $Y_0$ is increasing.

Suppose that $y_k = \max_j y_j$. Note that $x_n = \max_i x_i$. For any two indices $i\neq n$, $j\neq k$, consider two equations

$$x_n \cdot y_k + x_i \cdot y_j$$ and $$x_n \cdot y_j + x_i \cdot y_k.$$
...
we obatin $x_n \cdot y_n + x_i \cdot y_j \geq x_n \cdot y_j + x_i \cdot y_n$ (equality holds only when $x_n = x_i$ and $y_k = y_j$). Since $x_i$, $y_j$ are arbitrary, $x_n$ should be multiplied with $y_k = \max_j y_j$.

Hence, we may assume $n = k$ and so $\max_Y \langle X, Y \rangle = x_n \cdot y_n + \max_{Y'}\langle X', Y' \rangle$, where $X' = (x_1, x_2, \ldots, x_{n-1})$ and $Y' = (y_1, y_2, \ldots, y_{n-1})$. Then our claim will be done recursively (or by mathematical induction).
