...
피어슨 상관계수와 굉장히 달라보이지만 실은 변수를 rank로 바꾼 후 피어슨 상관계수를 계산한 것과 같다.

---

**_proof_**

Note that both $X$ and $Y$ have size $n$. We may assume that both $X$ and $Y$ are already rank ordered. Consider

$$ \rho(X, Y) := \langle \frac{X - \mu(X)}{\sigma(X)}, \frac{Y - \mu(Y)}{\sigma(Y)} \rangle $$.

Since $X$ and $Y$ are permutations of $[n] = \{1, 2, \ldots, n\}$, we get $\mu := \mu(X) = \mu(Y) = \frac{n+1}{2}$. Also, $\sigma^2(X) = \simga^2(Y) = \sum\limits_{i=1}^n (X_i - \mu)^2$
