...
하지만 단순히 내적을 취하는 방식은 $X$와 $Y$ 값이 달라질 때마다 다른 기준치를 갖게 된다. 예를 들어, 
...
이러한 문제점을 해결하기 위해 각각의 변수에 대한 **평균값** 혹은 **기대값** $\mu$를 계산하여 빼주는 방법이 있다. 
...
$$\sigma(X) = \sqrt{\sum\limits_{i=1}^n x_i^2} = \left\lVert X \right\rVert.$$

Similarly, $\sigma(Y) =  \left\lVert Y \right\rVert $.

Note that $Y = aX$ implies $\sigma(Y) = |a| \sigma(X)$. Then if $a \neq 0$,

$$
\begin{aligned}
a \big(\frac{X}{\sigma(X)}\big) &= a \cdot \frac{|a|X}{|a| \sigma(X)}
\\ &= |a| \cdot \frac{aX}{|a| \sigma(X)}
\\ &= |a| \big(\frac{Y}{\sigma(Y)}\big).
\end{aligned}
$$
...
$X = (x_1, x_2, \ldots, x_n)$, $Y = (y_1, y_2, \ldots, y_n)$이라 하여 $\rho$를 풀어쓰면 다음과 같은 식을 얻을 수 있다.

$$
\rho(X, Y) = \frac
{
  \sum\limits_{i=1}^n \big(x_i - \mu(X) \cdot (y_i - \mu(Y)\big)
}
{
  \sqrt{
...
위의 관점을 이용하면, 피어슨 상관계수를 3개 이상의 변수일 경우에도 일반화 가능할 것이다.
...
Let $X_1, X_2, \ldots, X_n$ be i.i.d. random variables. Our claim is that $\text{rk}(X_1), \text{rk}(X_2), \ldots, \text{rk}(X_n)$ are uniform distributions.
...
$$X, Y \leftarrow \text{rk}(X), \text{rk}(Y).$$
...
Note that since the data $X$ and $Y$ are from i.i.d., our ranked data form a discrete uniform distribution. Hence,

$$ \mu(X) = \frac{1}{n} \sum\limits_{i=1}^n x_i^2 = \frac{n+1}{2} = \mu(Y) $$

and

$$
\begin{aligned}
\sigma(X) &= \sqrt{\sum\limits_{i=1}^n (x_i - \mu)^2}
\\ &= \sqrt{\sum\limits_{i=1}^n x_i^2 - 2\mu \sum\limits_{i=1}^n x_i + n\mu^2}
\\ &= \sqrt{\sum\limits_{i=1}^n x_i^2 - n\mu^2}
\\ &= \sqrt{\frac{n(n+1)(2n+1)}{6} - n\mu^2}
\\ &= \sqrt{\frac{n(n+1)(2n+1)}{6} - n(\frac{n+1}{2})^2}
\\ &= \sqrt{n(n+1)(\frac{2n+1}{6} - \frac{n+1}{4})}
\\ &= \sqrt{\frac{n(n+1)(n-1)}{12}}
\\ &= \sigma(Y)
\end{aligned}
$$
