...
$$x_n \cdot y_k + x_i \cdot y_j \text{ and } x_n \cdot y_j + x_i \cdot y_k$$
...
하지만 단순히 내적을 취하는 방식은 $X$와 $Y$ 값이 달라질 때마다 다른 척도가 된다. 예를들어, 

$$ X = (1, 2, 3)\\ Y_1 = (4, 5, 6)\\ Y_2 = (7, 8, 9) $$

에서 $X$와 $Y_1$, 그리고 $X$와 $Y_2$의 관계는 둘 다 선형적인 양의 관계이다. 우리는 두 경우 모두 상관관계를 계산했을 때, 동일한 값을 갖게 하고 싶지만 내적을 계산해보면 다른 값을 갖게 된다.

- $\langle X, Y_1 \rangle = 1*4 + 2*5 + 3*6 = 32$
- $\langle X, Y_2 \rangle = 1*7 + 2*8 + 3*9 = 50$
