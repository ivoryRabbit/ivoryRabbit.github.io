...
이러한 문제점을 해결하기 위해, 각각의 변수에 대한 **평균값**을 계산하여 빼주는 방법이 있다. 즉, $\rho(X, Y) = \langle X - \mu(X), Y - \mu(Y) \rangle$으로 정의하는 것이다.
...
각각의 평균을 빼는 것으로 변수를 업데이트해주면, $Y = aX$가 되며 이는 곧

$$ \langle X, Y \rangle = \langle X, aX \rangle = a\left\lVert X \right\rVert $$

가 된다. 즉, $\rho$가 $X$와 $Y$의 선형관계에 대한 기울기를 측정해주는 것이다.

위의 예에서 $X$, $Y_1$, $Y_2$를 평균으로 각각 뺀후, 내적을 하는 방법으로 상관계수를 구하면 다음을 얻는다.

$$ X = Y_1 = Y_2 = (-1, 0, 1)$$

$$ \lrange X, Y_1 \rangle = 2 = \lrange X, Y_1 \rangle$$

마지막으로 지금의 $\rho$는 기울기 혹은 변수의 크기에 영향을 받고 있다. 예를 들어
