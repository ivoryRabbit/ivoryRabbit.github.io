...
상관계수란 두 변수(variable)가 ordinal scale로 주어졌을 때 둘 사이에 얼마나 상관관계가 있는지 측정하는 척도이다. 보통 양의 상관관계를 가질 때 상관계수가 크고, 음의 상관관계를 가질 때 상관계수가 작도록 설계되어 있다. 여기서 상관관계가 반드시 "선형적이다"의 의미를 포함하진 않지만 가장 많이 사용되는 피어슨 상관계수가 이 선형성(linearlity)을 측정하고 있다.

상관계수는 이미 잘 알려져 있는 상관분석(correlation)은 물론이고, 회귀분석(regression)에서 결정계수(coefficient of determination)를 계산할 때도 사용된다. 또한 추천시스템(recomendation system)의 협력 필터링(collaborative filtering)에서 두 아이템 혹은 두 고객의 유사도(similarity)를 계산할때나 추천이 얼마나 성공적인지 계산하는 평가함수(evaluation function)로써 사용된다.
...
$$ \rho_s := 1 - \frac{6 \left\lVert rk(X) - rk(Y) \right\rVert^2}{n(n^2-1)} $$
