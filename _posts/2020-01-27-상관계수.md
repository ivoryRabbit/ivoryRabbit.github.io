...
$$ \rho(X, Y_1) = \langle X, Y_1 \rangle = 1\dot 4 + 2\dot 5 + 3\dot 6 = 32 $$

$$ \rho(X, Y_2) = \langle X, Y_2 \rangle = 1\dot(-2) + 2\dot(-1) + 3\dot 0 = -4 $$
...
$$ \rho(X, Y_1) = \langle X, Y_1 \rangle = (-1)\dot (-2) + 1\dot 2 = 4 $$

$$ \rho(X, Y_2) = \langle X, Y_2 \rangle = (-1)\dot (-3) + 1\dot 3 = 6 $$
...
우리는 이러한 함수 $\rho$를 **피어슨 상관계수**라고 부른다. 피어슨 상관계수는 두 unit vector가 이루는 각도의  값이기 때문에 $-1$에서 $1$ 사이의 값을 가진다. (이는 Cauchy-Schwarz 부등식으로도 증명 가능하다.)
