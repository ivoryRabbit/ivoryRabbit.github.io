...
Let $X = (x_1, x_2, \ldots, x_n)$ and $Y = (y_1, y_2, \ldots, y_n)$ be given. We will show that the inner product $\langle X, Y \rangle$ of $X$ and $Y$ is the maximum when $X$ and $Y$ are both increasing.

Suppose that $x_i \leq x_j$ and $y_i \leq y_j$ for all $i<j$, that is, increasing. Note that $x_n$ an $y_n$ are the greatest numbers of $X$ and $Y$, respectively. For any $i,j < n$, consider two equations $x_n \cdot y_n + x_i \cdot y_j$ and $x_n \cdot y_j + x_i \cdot y_n$. By the following, we obatin $x_n \cdot y_n + x_i \cdot y_j \geq x_n \cdot y_j + x_i \cdot y_n$
